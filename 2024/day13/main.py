...
def first_part():
    ...
    for cm in claw_machines:
        ...
        solution = np.linalg.solve(equations, other_side).astype(int)
        ...
        print(f"Using {cm} and getting {solution}")
        print(f"{solution[0] * cm.button_a.x + solution[1] * cm.button_b.x} == {cm.prize[0]}")
        print(f"{solution[0] * cm.button_a.y + solution[1] * cm.button_b.y} == {cm.prize[1]}")
...
def first_part_2():
    ...
    for cm in claw_machines:
        ...
        if min_tokens is not None:
            tokens += min_tokens
    print(f"Numer of tokesn {tokens}")
